...
def parse_args(input_args=None):
    ...
    # Anchor-based Retention (Idea 1)
    parser.add_argument(
        "--anchor_lambda",
        type=float,
        default=0.0,
        help="Weight of the anchor retention loss (DSM on safe-looking concepts).",
    )
    parser.add_argument(
        "--anchor_dir",
        type=str,
        default="",
        help="Optional override for the directory holding anchor images.",
    )
    parser.add_argument(
        "--anchor_t_min",
        type=int,
        default=1,
        help="Lower bound of timestep range sampled for the anchor loss.",
    )
    parser.add_argument(
        "--anchor_t_max",
        type=int,
        default=750,
        help="Upper bound of timestep range sampled for the anchor loss.",
    )
    parser.add_argument(
        "--dcoloss_beta",
        type=float,
...
class TrainDataset(Dataset):
    def __init__(self, args):
        ...
        if (
            ...
            and "anchor_images" in data_cfg
            and "anchor_prompts" in data_cfg
        ):
            anchor_dir_name = data_cfg["anchor_images"]
            anchor_image_dir = (
                args.anchor_dir
                if args.anchor_dir
                else os.path.join(self.data_dir, self.config_name, anchor_dir_name)
            )
            if os.path.isdir(anchor_image_dir):
                anchor_paths = sorted(
                    [
                        p
                        for p in os.listdir(anchor_image_dir)
                        if p.lower().endswith((".jpg", ".jpeg", ".png"))
                    ]
                )
                anchor_prompts_cfg = data_cfg["anchor_prompts"]
                # Distribute prompt list round-robin over image list
                if isinstance(anchor_prompts_cfg, list):
                    for i, p in enumerate(anchor_paths):
                        self.anchor_images.append(
                            Image.open(os.path.join(anchor_image_dir, p))
                        )
                        self.anchor_prompts.append(
                            anchor_prompts_cfg[i % len(anchor_prompts_cfg)]
                        )
                print(
                    f"[anchor] loaded {len(self.anchor_images)} anchor images "
                    f"from {anchor_image_dir}"
                )
            else:
                print(
                    f"[anchor] WARNING: anchor dir not found: {anchor_image_dir}"
                )
        self.num_anchor_images = len(self.anchor_images)
    ...
    def __getitem__(self, index):
        ...
        # anchor image (Idea 1)
        if self.num_anchor_images > 0:
            ai = index % self.num_anchor_images
            anchor_image = self.anchor_images[ai]
            anchor_image = exif_transpose(anchor_image)
            if not anchor_image.mode == "RGB":
                anchor_image = anchor_image.convert("RGB")
            example["anchor_images"] = self.image_transforms(anchor_image)
            example["anchor_prompt"] = self.anchor_prompts[ai]
        else:
            # placeholder (won't be used when anchor_lambda == 0)
            example["anchor_images"] = example["instance_images"]
...
def collate_fn(examples, args):
    ...
    anchor_pixel_values = [example["anchor_images"] for example in examples]
    ...
    anchor_pixel_values = torch.stack(anchor_pixel_values)
    anchor_pixel_values = anchor_pixel_values.to(
        memory_format=torch.contiguous_format
    ).float()
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                anchor_prompts = batch["anchor_prompts"]
                ...
                anchor_pixel_values = batch["anchor_pixel_values"].to(dtype=vae.dtype)
                ...
                # ----- Anchor Retention Loss (Idea 1) -----
                # DSM on anchor images at multiple timesteps, anchoring the
                # unlearned LoRA to behave like the reference model on
                # visually-similar safe concepts.
                anchor_loss_val = 0.0
                if (
                    args.anchor_lambda > 0.0
                    and train_dataset.num_anchor_images > 0
                ):
                    with torch.no_grad():
                        anchor_latents = (
                            vae.encode(anchor_pixel_values).latent_dist.sample()
                            * vae.config.scaling_factor
                        ).to(weight_dtype)
                    ab = anchor_latents.shape[0]
                    anchor_noise = torch.randn_like(anchor_latents)
                    a_t_min = max(args.anchor_t_min - 1, 0)
                    a_t_max = min(args.anchor_t_max,
                                  noise_scheduler.config.num_train_timesteps)
                    anchor_timesteps = torch.randint(
                        a_t_min, a_t_max, (ab,),
                        device=anchor_latents.device,
                    ).long()
                    noisy_anchor = noise_scheduler.add_noise(
                        anchor_latents, anchor_noise, anchor_timesteps
                    )
                    anchor_prompt_embeds, _ = pipe.encode_prompt(
                        anchor_prompts,
                        device=accelerator.device,
                        num_images_per_prompt=1,
                        do_classifier_free_guidance=False,
                    ...
                    anchor_pred = unet(
                        noisy_anchor, anchor_timesteps, anchor_prompt_embeds
                    ).sample
                    anchor_loss = F.mse_loss(
                        anchor_pred.float(), anchor_noise.float(), reduction="mean"
                    )
                    loss = loss + args.anchor_lambda * anchor_loss
                    anchor_loss_val = anchor_loss.detach().item()
            ...
            if accelerator.sync_gradients:
                ...
                if (
                    ...
                    logs["loss_anchor"] = anchor_loss_val
